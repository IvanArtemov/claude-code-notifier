...
После установки необходимо создать локальный конфигурационный файл с вашим Telegram Chat ID:

**Создайте файл `.claude/settings.local.json`:**
```json
{
  "env": {
...
Этот файл уже добавлен в `.gitignore` и не попадет в репозиторий. Используйте `.claude/settings.local.json.example` как шаблон.

**Как получить Chat ID:**
1. Запустите бота (через `/notifier-start` или вручную)
2. Откройте вашего бота в Telegram
3. Отправьте команду `/start`
4. Бот пришлет ваш Chat ID
